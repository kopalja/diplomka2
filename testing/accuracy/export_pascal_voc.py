...
def test_xml(xml_path, searching_objects):
    # private function
    def base_coordinates(mmin, mmax, size):
        dx = (300 - (mmax - mmin)) // 2
        or_min = mmin
        or_max = mmax
        mmin -= dx
    ...
    if (int(data['size']['width']) < 300) or (int(data['size']['height']) < 300):
        return False, []
    ...
    if interesting_object_exist and (x_max - x_min <= 300) and (y_max - y_min <= 300):
        x_min, x_max = base_coordinates(x_min, x_max, int(data['size']['width']))
        y_min, y_max = base_coordinates(y_min, y_max, int(data['size']['height']))
...
if __name__ == "__main__":
    ...
    dst_folder = os.path.join(os.environ['LOCAL_GIT'], 'testing/exported', 'pascal')
    ...
    searching_objects = ["bus", "car", "motorbike"]
    ...
    for xml_name in get_files(xmls_folder):
        ...
        passed, coordinates = test_xml(xml_path, searching_objects)
        if passed:
            print(coordinates)
